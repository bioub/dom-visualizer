...
d3.domVisualizer.prototype.parseHTML = function(htmlStr) {
    ...
    if(oldTree) {
        ...
        removes.forEach(function(node) {
            node.par.children.splice(node.childidx, 1);
            var idx = 0;
            node.par.children.forEach(function(node) {
                node.childidx = idx++;
            })
        });
        ...
        inserts.forEach(function(node) {
            node.par.partner.children.splice(node.childidx, 0, node);
            node.par = (node.par.partner) ? node.par.partner : node.par;
            node.forEach(function(n) {
                n.parent = n.par;
            });
            var idx = 0;
            node.par.partner.children.forEach(function(node) {
                node.childidx = idx++;
            });
        });
